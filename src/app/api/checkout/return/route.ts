...
export async function POST(request: Request) {
  ...
  if (!paymentIntentId || !PAYMENT_INTENT_PATTERN.test(paymentIntentId)) {
    return NextResponse.json(
      { error: "Invalid PaymentIntent reference." },
      { status: 400 },
    );
  }
  ...
  if (!UUID_PATTERN.test(orderId)) {
    return NextResponse.json(
      { error: "Payment authorization is not linked to an order." },
      { status: 400 },
    );
  }
  ...
  if (!orderRaw || !canAccessOrder(access, orderRaw)) {
    return NextResponse.json(
      { error: "Payment does not match an active order." },
      { status: 400 },
    );
  }
  ...
  try {
    return NextResponse.json(
      await finalizeCheckoutAuthorization({
        orderRaw,
        intent,
        request,
