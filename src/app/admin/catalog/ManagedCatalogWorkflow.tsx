...
import {
  ...
  hasAdvancedOnlySphereRules, hasAdvancedToricRules, lensTypeFromCatalogProductType,
  parametersFromAdvancedJson, parseNumberList, parsePriceToCents,
  shouldShowCatalogValidationIssue, suggestedCatalogCoreId, type CatalogProductType,
...
function NumberListField({ label, value, onChange, help, error }: {
  label: string; value: readonly number[] | undefined; onChange: (values: number[]) => void;
  help?: string; error?: string | null;
}) {
  ...
  return <label className={error ? "fieldError" : ""}>{label}
    ...
    }} placeholder="Example: 8.4, 8.6" />
    {help && <small>{help}</small>}{error && <small className="errorText">{error}</small>}
  </label>;
}

function SphereEditor({ parameters, onChange, error }: {
  parameters: LensCore["parameters"]; onChange: (parameters: LensCore["parameters"]) => void; error?: string | null;
}) {
  ...
  return <div className="ruleBlock"><div className="sectionLead"><div><h3>Sphere availability</h3><p>Add every manufacturer range. Each range can use its own increment.</p></div><button type="button" onClick={() => onChange({ ...parameters, sphere: { ...(sphere ?? {}), segments: [...segments, { min: -6, max: 6, step: 0.25 }] } })}>Add sphere range</button></div>
    ...
    <NumberListField key={"sphere-exclude:" + formatNumberList(sphere?.exclude)} label="Excluded sphere powers (optional)" value={sphere?.exclude} onChange={(exclude) => onChange({ ...parameters, sphere: { ...(sphere ?? {}), segments, ...(exclude.length ? { exclude } : {}) } })} help="Comma-separated powers that are not manufacturable within these ranges." error={error} />
  </div>;
}

function ToricEditor({ parameters, onChange, error }: {
  parameters: LensCore["parameters"]; onChange: (parameters: LensCore["parameters"]) => void; error?: string | null;
}) {
  ...
  return <div className="ruleBlock"><div className="sectionLead"><div><h3>Toric availability</h3><p>Use another cylinder group when manufacturer availability differs.</p></div><button type="button" onClick={() => onChange(addToricGroup(parameters))}>Add cylinder group</button></div>
    {groups.map((group, index) => <div className="toricGroup" key={index}><div className="sectionLead"><strong>Cylinder group {index + 1}</strong><button type="button" className="quietButton" disabled={groups.length === 1} onClick={() => onChange({ ...parameters, toric: { groups: groups.filter((_, itemIndex) => itemIndex !== index) } })}>Remove group</button></div>
      <NumberListField key={"cylinders:" + index + ":" + formatNumberList(group.cylinders)} label="Cylinder values" value={group.cylinders} onChange={(cylinders) => changeGroup(index, (current) => ({ ...current, cylinders } as ToricGroup))} help="Example: -0.75, -1.25, -1.75" />
      {hasAdvancedToricRules(group) ? <div className="ruleList"><p>Axis availability is restricted by sphere power for this group.</p>{group.sphereAxisRules.map((rule, ruleIndex) => <div className="sphereAxisRule" key={ruleIndex}><label>Sphere minimum<input type="number" step="0.25" value={rule.sphereRange.min} onChange={(event) => changeRule(index, ruleIndex, (current) => ({ ...current, sphereRange: { ...current.sphereRange, min: Number(event.target.value) } }))} /></label><label>Sphere maximum<input type="number" step="0.25" value={rule.sphereRange.max} onChange={(event) => changeRule(index, ruleIndex, (current) => ({ ...current, sphereRange: { ...current.sphereRange, max: Number(event.target.value) } }))} /></label><NumberListField key={"axis:" + index + ":" + ruleIndex + ":" + formatNumberList(rule.axis)} label="Available axes" value={rule.axis} onChange={(axis) => changeRule(index, ruleIndex, (current) => ({ ...current, axis }))} /><label>Sphere increment<select value={rule.sphereStepOverride ?? ""} onChange={(event) => changeRule(index, ruleIndex, (current) => event.target.value ? { ...current, sphereStepOverride: Number(event.target.value) as 0.25 | 0.5 | 1 } : withoutOverride(current))}><option value="">Use sphere increment</option><option value={0.25}>0.25 D</option><option value={0.5}>0.50 D</option><option value={1}>1.00 D</option></select></label><button type="button" className="quietButton" onClick={() => changeGroup(index, (current) => hasAdvancedToricRules(current) ? { ...current, sphereAxisRules: current.sphereAxisRules.filter((_, itemIndex) => itemIndex !== ruleIndex) } : current)}>Remove restriction</button></div>)}<button type="button" onClick={() => changeGroup(index, (current) => hasAdvancedToricRules(current) ? { ...current, sphereAxisRules: [...current.sphereAxisRules, { sphereRange: { min: -6, max: 6 }, axis: [] }] } : current)}>Add sphere/axis restriction</button></div> : <div className="axisCommon"><NumberListField key={"axis:" + index + ":" + formatNumberList(group.axis)} label="Axis values" value={group.axis} onChange={(axis) => changeGroup(index, (current) => ({ cylinders: current.cylinders, axis }))} help="Example: 10, 20, 30 … 180" /><button type="button" className="quietButton" onClick={() => changeGroup(index, (current) => ({ cylinders: current.cylinders, sphereAxisRules: [{ sphereRange: { min: -6, max: 6 }, axis: "axis" in current ? current.axis ?? [] : [] }] }))}>Add sphere/axis restrictions</button></div>}
    </div>)}
    {error && <small className="errorText">{error}</small>}
  </div>;
}

function MultifocalEditor({ parameters, onChange, error }: {
  parameters: LensCore["parameters"]; onChange: (parameters: LensCore["parameters"]) => void; error?: string | null;
}) {
  ...
  return <div className="ruleBlock"><div className="sectionLead"><div><h3>Multifocal availability</h3><p>ADD labels can be manufacturer names such as LOW, MED, and HIGH.</p></div><button type="button" onClick={() => onChange(addMultifocalGroup(parameters))}>Add ADD-specific restriction</button></div>
    <label>Available ADD values or categories<input value={multifocal.adds.join(", ")} onChange={(event) => onChange({ ...parameters, multifocal: { ...multifocal, adds: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) } })} placeholder="Example: LOW, MED, HIGH" /><small>Comma-separated labels shown to customers exactly as entered.</small></label>
    {groups.map((group, index) => <div className="multifocalGroup" key={index}><div className="sectionLead"><strong>ADD-specific rule {index + 1}</strong><button type="button" className="quietButton" onClick={() => onChange({ ...parameters, multifocal: { ...multifocal, groups: groups.filter((_, itemIndex) => itemIndex !== index) } })}>Remove rule</button></div><label>ADD values for this rule<input value={group.adds.join(", ")} onChange={(event) => changeGroup(index, (current) => ({ ...current, adds: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) }))} /></label><div className="powerRange"><label>Sphere minimum<input type="number" step="0.25" value={group.sphereRange.min} onChange={(event) => changeGroup(index, (current) => ({ ...current, sphereRange: { ...current.sphereRange, min: Number(event.target.value) } }))} /></label><label>Sphere maximum<input type="number" step="0.25" value={group.sphereRange.max} onChange={(event) => changeGroup(index, (current) => ({ ...current, sphereRange: { ...current.sphereRange, max: Number(event.target.value) } }))} /></label><label>Sphere increment<select value={group.sphereStepOverride ?? ""} onChange={(event) => changeGroup(index, (current) => event.target.value ? { ...current, sphereStepOverride: Number(event.target.value) as 0.25 | 0.5 | 1 } : withoutOverride(current))}><option value="">Use sphere increment</option><option value={0.25}>0.25 D</option><option value={0.5}>0.50 D</option><option value={1}>1.00 D</option></select></label></div></div>)}
    {error && <small className="errorText">{error}</small>}
  </div>;
}
...
export default function ManagedCatalogWorkflow({ initialFamilies, storageAvailable, startInCreateMode = false }: { initialFamilies: ManagedCatalogFamily[]; storageAvailable: boolean; startInCreateMode?: boolean }) {
  ...
  function add() {
    const next = emptyManagedCatalogFamily(); setSelectedId(null); setDraft(next); setCoreIdOverride(false); setAdvancedText(JSON.stringify(next.parameters, null, 2)); setAdvancedError(null); setTouched(new Set()); setPublishAttempted(false); setStatus(null); setImagePreviewUrl(null);
  }
  function select(family: ManagedCatalogFamily) {
    const next = asInput(family); setSelectedId(family.coreId); setDraft(next); setCoreIdOverride(true); setAdvancedText(JSON.stringify(next.parameters, null, 2)); setAdvancedError(null); setTouched(new Set()); setPublishAttempted(false); setStatus(null); setImagePreviewUrl(null);
  }
  ...
  return <section id="managed-catalog" className="managedCatalog" tabIndex={-1}>
    ...
    {!storageAvailable ? <p className="managedError">Lens catalog storage is not available. Nothing can be published from this page.</p> : <div className="managedGrid">
      ...
      <form className="managedForm" onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        <section><h3>2. Prescription parameters</h3><div className="two"><NumberListField key={"bc:" + formatNumberList(draft.parameters.baseCurve)} label="Base curve (BC)" value={draft.parameters.baseCurve} onChange={(baseCurve) => updateParameters((parameters) => ({ ...parameters, baseCurve }))} help="One or more discrete values, separated by commas." error={errorsFor("parameters.baseCurve")} /><NumberListField key={"dia:" + formatNumberList(draft.parameters.diameter)} label="Diameter (DIA)" value={draft.parameters.diameter} onChange={(diameter) => updateParameters((parameters) => ({ ...parameters, diameter }))} help="One or more discrete values, separated by commas." error={errorsFor("parameters.diameter")} /></div><SphereEditor parameters={draft.parameters} onChange={(parameters) => updateParameters(() => parameters)} error={errorsFor("parameters.sphere")} />{draft.type.toric && <ToricEditor parameters={draft.parameters} onChange={(parameters) => updateParameters(() => parameters)} error={errorsFor("parameters.toric")} />}{draft.type.multifocal && <MultifocalEditor parameters={draft.parameters} onChange={(parameters) => updateParameters(() => parameters)} error={errorsFor("parameters.multifocal")} />}<details className="advanced"><summary>Advanced prescription rules</summary><p>For exceptional manufacturer rules such as BC-dependent sphere ranges, use the complete availability model. Existing advanced rules remain unchanged unless you edit them here.</p><textarea value={advancedText} onChange={(event) => updateAdvanced(event.target.value)} spellCheck={false} aria-label="Advanced prescription rules" />{errorsFor("parameters") && <small className="errorText">{errorsFor("parameters")}</small>}</details></section>
