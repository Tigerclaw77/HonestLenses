  ...
  useEffect(() => {
    ...
    async function reconcile() {
      if (!paymentIntentId || !PAYMENT_INTENT_PATTERN.test(paymentIntentId)) {
        setError(
          "Payment return information is missing. Please use your secure resume-order link.",
        );
        return;
      }
      const {
      ...
      if (!response.ok || !body.ok || !body.orderId || !body.next) {
        setError(
          body.error ??
            "We could not finish checkout. Please use your secure resume-order link.",
        );
        return;
      }
      if (body.next === "verification-details") {
        router.replace(
          `/checkout/verification-details?orderId=${encodeURIComponent(body.orderId)}`,
        );
        return;
      }
      router.replace(
  ...
  return (
    <main>
      <section className="content-shell">
        ...
        {error && (
          <button onClick={() => router.replace("/resume-order")}>Resume an Order</button>
        )}
      </section>
    </main>
  );
