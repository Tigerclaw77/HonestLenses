...
function lookupPricing(sku: string): PricingLookupResult {
  if (sku in VISTAKON_PRICING) {
    return {
      ...
      entry: VISTAKON_PRICING[sku as keyof typeof VISTAKON_PRICING],
    };
  }
  ...
  if (sku in BAUSCH_PRICING) {
    return {
      manufacturer: "bausch",
      entry: BAUSCH_PRICING[sku as keyof typeof BAUSCH_PRICING],
    };
  }
  ...
  if (sku in COOPERVISION_PRICING) {
    return {
      ...
      entry: COOPERVISION_PRICING[sku as keyof typeof COOPERVISION_PRICING],
    };
  }
  ...
  if (sku in ALCON_PRICING) {
    return {
      manufacturer: "alcon",
      entry: ALCON_PRICING[sku as keyof typeof ALCON_PRICING],
    };
  }
